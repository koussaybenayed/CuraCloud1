...
class WebSocketServer implements MessageComponentInterface
{
    ...
    {
        // Start WebSocket server logic
        // Use $this->websocketUrl where needed
        echo "WebSocket server started at {$this->websocketUrl}\n";
    }
    ...
    private function broadcastStreamMessage(ConnectionInterface $from, string $content)
    {
        ...
        $this->publisher->__invoke(new Update(
           // '%env("http://localhost:3000/.well-known/mercure")%', // Replace with your actual URL
           getenv('MERCURE_URL'), // Replace with your actual environment variable name if different
            json_encode([
                'type' => 'stream_message',
                'userId' => $from->userId,
                'content' => $content,
