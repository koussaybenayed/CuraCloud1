...
public class AppointmentAddController {
    ...
    // Method to initialize the time spinners
    ...
    public void sendNotificationSMS() {
            ...
            String notificationMessage = "Réservation effectuée avec les détails suivants :\n" +
                    "- Description: %s\n" +
        ...
        try {
            if (validateInputs()) {
                ...
                if (selectedDoctor == null || selectedPatient == null) {
                    showAlert(Alert.AlertType.ERROR, "Validation Error", "Please select a doctor and a patient.");
                    return;
                }
                LocalDate appointmentDateValue = appointmentDate.getValue();
                LocalDateTime startDateTime = combineDateTime(appointmentDateValue, startTime1);
                LocalDateTime endDateTime = combineDateTime(appointmentDateValue, endTime1);
                ...
                if (isTimeSlotAvailable(startDateTime, endDateTime, idDoctor)) {
                    Appointment newAppointment = new Appointment(description, "pending", startDateTime, endDateTime, 0 , idPatient, idDoctor);
                    ...
                    List<LocalDateTime> availableSlots = getAvailableSlots(appointmentDateValue);
